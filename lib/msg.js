...
exports.smsg = async (client, message, store, returnClient = false) => {
  ...
  if (message.message) {
    ...
    if (serializedMsg.mtype === "viewOnceMessage" || serializedMsg.mtype === "viewOnceMessageV2" || serializedMsg.msg?.viewOnce) {
      serializedMsg.viewOnce = true;
      if (!serializedMsg.mtype2 || serializedMsg.mtype2 === serializedMsg.mtype) {
        const inner = message.message?.viewOnceMessage?.message || message.message?.viewOnceMessageV2?.message;
        if (inner) {
          serializedMsg.mtype2 = Object.keys(inner)[0];
          serializedMsg.msg = inner[serializedMsg.mtype2];
          serializedMsg.msg.mtype = serializedMsg.mtype2;
        }
      }
    }
    ...
    if (quotedContent) {
      ...
      if (serializedMsg.quoted.message) {
        ...
        if (serializedMsg.quoted.jid === 'status@broadcast' || serializedMsg.quoted.key?.remoteJid === 'status@broadcast' || serializedMsg.msg?.contextInfo?.remoteJid === 'status@broadcast') {
