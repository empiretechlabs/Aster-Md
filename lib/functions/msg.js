...
const { proto, downloadContentFromMessage, getContentType, WA_DEFAULT_EPHEMERAL } = require('baileys-pro')
const fs = require('fs')
const fetch = (...args) => import('node-fetch').then(({default: fetch}) => fetch(...args));
const axios = require("axios")
const { fileTypeFromBuffer } = require("file-type") 
const { writeExif, imageToWebp, videoToWebp, writeExifWebp, writeExifImg, writeExifVid } = require('./exif.js');
...
const sms = (conn, m) => {
  ...
  m.sendstk = async (content, options = {packname: "Empire", author: "Md" } ) => {
  let filename, buffer;
  try {
    buffer = Buffer.isBuffer(content) ? content : /^data:[^;]+;base64,/i.test(content) ? Buffer.from(content.split(',')[1], 'base64') : /^https?:\/\//.test(content) ? await getBuffer(content) : fs.existsSync(content) ? (filename = content, await fs.promises.readFile(content)) : null;
    if (!buffer || buffer.length === 0) throw new Error('Invalid content: must be a Buffer, Base64 data URI, URL, or file path');
  } catch (error) { throw new Error(`Failed to process content: ${error.message}`); }
  let mime;
  try { ({ mime } = await fileTypeFromBuffer(buffer)); } catch (error) { throw new Error(`Failed to detect mime type: ${error.message}`); }
  try {
    const chat = m.chat, hasMetadata = options.packname || options.author, messageOptions = { ...options, ephemeralExpiration: WA_DEFAULT_EPHEMERAL }, quoted = { quoted: options.quoted };
    if (mime.includes('webp')) {
      const stickerUrl = await writeExifWebp(buffer, { packname: options.packname, author: options.author || (options.packname ? '' : ' ') });
      return await conn.sendMessage(chat, { sticker: { url: stickerUrl }, ...messageOptions }, quoted);
    }
    const isImage = mime.includes('image'), convertFn = isImage ? imageToWebp : videoToWebp, exifFn = isImage ? writeExifImg : writeExifVid;
    return await conn.sendMessage(chat, { sticker: hasMetadata ? { url: await exifFn(buffer, { packname: options.packname, author: options.author }) } : await convertFn(buffer), ...messageOptions }, quoted);
  } catch (error) { throw new Error(`Failed to send sticker: ${error.stack}`); }
};
  ...
  m.replyAud = (aud, id = m.chat, option = { mentions: [], ptt: false }) => conn.sendMessage(id, { audio: aud, ptt: option.ptt, mimetype: 'audio/mpeg', contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
