...
async function aitts(msg, text = '', showError = true) {
  try {
    ...
    let voiceIndex = Math.floor(Math.random() * voiceIds.length);
    if (!isNaN(voiceIdIndex) && voiceIdIndex >= 1 && voiceIdIndex <= voiceIds.length) {
      voiceIndex = voiceIdIndex - 1;
    }
    ...
    if (showError) {
      ...
      if (apiError && (Buffer.isBuffer(apiError) || apiError instanceof ArrayBuffer)) {
        try {
          const parsed = JSON.parse(Buffer.from(apiError).toString());
          apiError = parsed?.detail?.message || parsed?.detail || JSON.stringify(parsed);
        } catch {}
      }
      await msg.error((apiError || error.message) + "\n\ncommand: aitts", error);
    }
  }
}
