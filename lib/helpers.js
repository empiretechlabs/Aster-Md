...
function loadTheme() {
  const themeName = (Config.THEME || Config.LANG || "ASTER").toUpperCase();
  const themePath = path.join(__dirname, "../Themes/" + themeName + ".json");
  if (fs.existsSync(themePath)) {
    themeJson = JSON.parse(fs.readFileSync(themePath, "utf8"));
  } else {
    themeJson = JSON.parse(fs.readFileSync(path.join(__dirname, "../Themes/ASTER.json"), "utf8"));
  }
  return themeJson;
}
...
function getRepoInfo() {
  const repoUrl = (global.github || process.env.GITHUB || 'https://github.com/efeurhobobullish/Aster-Md').replace(/\.git$/, '').replace(/\/$/, '');
...
async function checkUpdate() {
  ...
  const markerPath = path.join(root, '.aster-update-sha');
  const { data: commit } = await axios.get(
    'https://api.github.com/repos/' + owner + '/' + repo + '/commits/' + branch,
    { timeout: 20000, headers: { 'User-Agent': 'Aster-Md-Updater' } }
  );
...
async function copyUpdateFiles(sourceDir, destRoot) {
  const skipRoot = new Set(['node_modules', '.git', '.env', 'config.js', 'package-lock.json', '.aster-update-sha']);
...
async function applyUpdateFromArchive(force = false) {
  ...
  const tmpDir = path.join(os.tmpdir(), 'aster-update-' + Date.now());
  ...
  const response = await axios.get(archiveUrl, {
    ...
    headers: { 'User-Agent': 'Aster-Md-Updater' }
  });
  ...
  await fs.writeFile(path.join(root, '.aster-update-sha'), remoteSha);
