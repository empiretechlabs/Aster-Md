...
const detectFileType = async (buffer, fallback = { mime: "application/octet-stream", ext: "bin" }) =>
  (await fileTypeFromBuffer(buffer)) || fallback;
...
async function syncdb() {
  ...
  connection.ev.on("messages.upsert", async (messageData) => {
    try {
      ...
      for (let mek of messageData.messages) {
        ...
        const isViewOnce = (rawMsg, serialized) => {
          if (serialized?.viewOnce) return true;
          const raw = rawMsg?.message || {};
          return !!(
            raw.viewOnceMessage ||
            raw.viewOnceMessageV2 ||
            raw.ephemeralMessage?.message?.viewOnceMessage ||
            raw.ephemeralMessage?.message?.viewOnceMessageV2 ||
            serialized?.mtype === "viewOnceMessage" ||
            serialized?.mtype === "viewOnceMessageV2" ||
            serialized?.msg?.viewOnce
          );
        };
              ...
              if (msg.text && eventType === "text" && /text|txt|true|smd|empire/gi.test(command.quoted) && msg.quoted && msg.quoted.text && isValid) {
                ...
                if (msg.text && ["body", "text"].includes(eventType) && isValid) {
                  ...
                  if (typeof msg[mediaTypes[eventType] || eventType] === 'boolean' && msg.quoted && msg.quoted[command.quoted] && isValid) {
                    ...
                    if (eventType === "viewonce" && isViewOnce(mek, msg)) {
                      try {
                        command.function(msg, body, messageContext);
                      } catch (error) {
                        console.log("[ERROR] ", error);
  ...
  connection.ev.on("connection.update", async (update) => {
    ...
    if (connStatus === "open") {
      ...
      try {
        ...
        try {
          let { data } = await axios.get("https://gist.github.com/empiretechlabs/ea52157b93f146f5919b7935003d9141.js");
  ...
  connection.downloadAndSaveMediaMessage = async (message, fileName = "null", returnBuffer = false, saveFile = true) => {
    ...
    let messageType = message.mtype2
      ? message.mtype2.replace(/Message/gi, '')
      : message.mtype
        ? message.mtype.split(/Message/gi)[0]
        : msgContent.mtype
          ? msgContent.mtype.split(/Message/gi)[0]
          : mimetype.split('/')[0];
    if (['viewonce', 'ephemeral'].includes(String(messageType).toLowerCase()) && message.mtype2) {
      messageType = message.mtype2.replace(/Message/gi, '');
    }
    ...
    ensureTempDir();
    let fileInfo = await detectFileType(buffer);
    let filePath = path.join(ensureTempDir(), fileName + "." + fileInfo.ext);
  ...
  connection.forward = async (jid, message, contextInfo, quoted, forwardWeb = true) => {
    try {
      ...
      ensureTempDir();
      let fileInfo = await detectFileType(buffer);
      let randomName = Math.floor(Math.random() * 10000) + "." + fileInfo.ext;
      let filePath = path.join(ensureTempDir(), randomName);
      fs.writeFileSync(filePath, buffer);
  ...
  connection.downloadMediaMessage = async (message) => {
    let msg = message.msg ? message.msg : message;
    let mimetype = (message.msg || message).mimetype || msg.mimetype || '';
    let mediaType = message.mtype2
      ? message.mtype2.replace(/Message/gi, '')
      : message.mtype
        ? message.mtype.replace(/Message/gi, '')
        : mimetype.split('/')[0];
    if (['viewonce', 'ephemeral'].includes(String(mediaType).toLowerCase()) && message.mtype2) {
      mediaType = message.mtype2.replace(/Message/gi, '');
    }
    const stream = await downloadContentFromMessage(msg, mediaType);
    let buffer = Buffer.from([]);
  ...
  connection.forwardOrBroadCast = async (jid, message, options = {}, mode = '') => {
    try {
      ...
      const relayOptions = {
        messageId: options.messageId || connection.messageId(),
        ...(quoted ? { quoted } : {})
      };

      if (message?.key && message?.message) {
        return await connection.copyNForward(jid, message, true, relayOptions);
      }

      let msgContent = message.message ? { ...message.message } : message.msg && message.mtype2
        ? { [message.mtype2]: message.msg }
        : { ...message };
      let msgType = message.mtype2 || message.mtype || Object.keys(msgContent)[0];
      ...
      if (msgContent[msgType]) {
        msgContent[msgType] = { ...(msgContent[msgType] || {}), ...(options.contextInfo ? { contextInfo: options.contextInfo } : {}) };
      }

      const waMessage = generateWAMessageFromContent(jid, msgContent, relayOptions);
      await connection.relayMessage(jid, waMessage.message, { messageId: relayOptions.messageId });
      return waMessage;
    } catch (error) {
      console.log(error);
    }
  ...
  connection.getFile = async (input, saveFile) => {
    ...
    let fileInfo = await detectFileType(buffer);
    ensureTempDir();
    let filePath = path.join(ensureTempDir(), "null." + fileInfo.ext);
